...
**What belongs in this file, for whoever edits it next:** would a round need it with no lifecycle around it, and would a solo shaped task need it with no round? Both, and it lives here; only one, and it belongs in that one — `team`, or whatever layer this workspace puts over either.
...
- **Plan** agrees scope before code. Iteration-sized work gets a plan doc the user reviews; a small fix gets a sentence or two. This is where the user's input concentrates: constraints, priorities, taste. Name the top risks and, for anything that ships behaviour, **the lever that undoes it** — a flag, a revert, a config toggle. A change whose rollback is "work out what to do when it breaks" has not been planned, and the answer is cheapest to find while the code does not exist yet.
...
**`pre-commit run --files "$FILES"` with a newline-joined variable matches nothing and exits 0.** Under zsh, which does not word-split unquoted expansions, the whole blob is one nonexistent path: every hook reports "(no files to check) Skipped" and the run reads as a pass. Pass a real argument list (`$(cat pathsfile)`, `xargs`, or an explicit array) and confirm the hook output names your files. **The second form bites even with the arguments right:** scoped hooks legitimately skip files outside their scope, so a wall of Skipped lines is not by itself evidence of a no-op — grep the skips out and confirm that at least the hooks which *should* have matched actually ran.
...
- **Iterate on a targeted subset at a high trial count, not the whole suite once.** When a change needs *resolution* on a handful of cases — is this failure real or a coin flip? — a broad run buys breadth you already have and spends hours doing it. Run everything once before merge.
...
- An interface in a language the round has not been working in → **a reviewer writing in that language**. Translated strings can be correct and still read as translated; only someone thinking in the language catches a compound spelled as two words, a term nobody uses, or a sentence that is grammatical and wrong. Worth calling now and then even when nothing changed, since copy drifts one string at a time.
...
**Pick each subagent's tier deliberately rather than inheriting the session's, and the discriminator is not the role — it is whether the output is checkable by something that is not another model.** Where the work is verified deterministically — a grep that must come back empty, a test suite, hooks, a build — a weaker tier is fine, because being wrong is caught immediately and for free: a mechanical rename sweep whose correctness a clean grep establishes is that case. Where the output is a judgement nothing mechanical can check — a design, an assessment, a review, "is this the right shape" — pay for the strong tier: an architecture review's findings are verifiable by nothing but another reading, and being wrong costs a round trip plus the rework, usually invisibly until much later. **The orchestrator is always the strong tier**: it holds scope and decides what reaches the user, and neither is checkable.

How that lands on today's tiers: mechanical, well-specified work whose correctness a gate establishes (scaffolds, fixtures, config, sweeps) → Haiku-class; standard implementation and research → Sonnet-class; algorithm-critical or judgment-heavy pieces that can't stay inline → Opus-class. **Never run subagents on the frontier tier (currently Fable-class)**: it costs and weighs on usage limits roughly double Opus-class, runs slower, and Opus-class matches or beats it on much of this work — if the frontier premium is worth paying anywhere, it's in the main loop. The independent verification above is what makes the smaller tiers safe. The discriminator is the rule; this mapping is only where it currently lands.

This is reasoning rather than measurement, and should not be cited as a validated result: usage history shows almost no use of the weaker tiers, which is consistent both with the test being right and with the alternative never having been tried.
