...
- **They are hidden from search too, and that is load-bearing rather than tidy.** A grep for a variable NAME is how a secret actually reaches a transcript — nobody was reading, so the rule above could not cover it. `~/.ignore` (tracked in dotfiles) carries the patterns, the `<filename>.example` template rule that pays back the lost discoverability, and the way back in; read it when adding a secrets file or a variable to one.
