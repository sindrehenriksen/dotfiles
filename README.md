...
- **gh CLI** uses the same pattern: `gh-personal` / `gh-work` shell functions point at `~/.config/gh-personal` / `~/.config/gh-work`; bare `gh` follows `$GH_DEFAULT_ACCOUNT` (default `personal`). Per-process env vars mean concurrent sessions don't stomp each other. One-time setup must use `--insecure-storage` so the OAuth token is written to each dir's `hosts.yml` (chmod 600) instead of the system keyring — the keyring is keyed by host only, so two `gh auth login`s share one entry and the last login wins for both accounts: `GH_CONFIG_DIR=~/.config/gh-<account> gh auth login --insecure-storage`.
