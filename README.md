...
- **Skills** in `.agents/skills/` cover: `pr-description`, `ci-debugging`, `browser`, `coderabbit`, `orchestration`, `sync`. Symlinked into both Codex and Claude locations via `install_symlinks.sh`. (Work-only skills such as `confluence` live in a separate private repo.)
