...
- **Skills** in `.agents/skills/` cover: `pr-description`, `ci-debugging`, `browser`, `coderabbit`, `execution`, `sync`. Symlinked into both Codex and Claude locations via `install_symlinks.sh`. (Work-only skills are kept out of this repo and linked in separately — the pattern for that, and for tiering agent config across a public and a private repo generally, is in `docs/agent-tiers.md`.)
