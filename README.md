...
| `system/` | Linux-only: GNOME keybinds, logind, battery conservation, systemd user-manager OOM policy, self-built mainline kernel — see `system/README.md` |
...
- **Skills** live in `agents/skills/`; the `generic_skills` array in `install_symlinks.sh` is the list of those every account gets, and linking them into the Codex and Claude locations is what that script does. Skills that are not generic — repo-local ones for working in this repo — are linked into its own `.claude/skills/` instead, and are gitignored there. (Skills specific to a project, system or employer are kept out of this repo and linked in separately — the pattern for that, and for composing this repo with a more specific setup generally, is in `docs/overlays.md`.)
