...
- [ ] Set `hideVimModeIndicator: true` in `.claude/settings.json` once anthropics/claude-code#77909 ships, to remove the duplicate `-- INSERT --` line below the prompt (statusline.sh already renders `[N]`/`[I]`/`[V]` from `.vim.mode`). Re-file chain: stale-closed #53556 → #66319 → #77909.
