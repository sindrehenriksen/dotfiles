...
- **No tap-to-switch-tabs, unlike macOS.** A dual-function modifier in xremap stops carrying its modifier into pointer events, so it breaks `Ctrl+click` and `Shift+click` while leaving every keyboard shortcut working — which is a nasty way to lose a browser gesture. Whether kanata's tap-hold has the same flaw is untested; it emits through a virtual device too, so do not assume it is immune.
